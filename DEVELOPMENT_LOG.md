...
IntelliJob/User/Home.aspx: updated featured jobs repeater to use modern card layout matching JobListing.aspx with logo, title, company meta, job pills, and view details button.
IntelliJob/User/Home.aspx: added modern job card styling (job-card, job-logo, job-title, job-meta, job-pills, btn-action) to match JobListing.aspx aesthetics.
...
IntelliJob/User/RegisterCompany.aspx.cs: now stores company passwords with salted SHA-256 hashing during registration.
IntelliJob/User/Login.aspx.cs: fixed login rehash flow by caching reader values before closing the reader and upgrading legacy plaintext passwords safely.
IntelliJob/Utils.cs: added `CreateSaltedHash` and `VerifyPassword` helpers (salted SHA-256 using existing ComputeSha256Hash and GenerateSalt).
IntelliJob/User/Register.aspx.cs: store password as salted SHA-256 hash using `Utils.CreateSaltedHash` on registration.
IntelliJob/User/Login.aspx.cs: verify passwords using `Utils.VerifyPassword`; re-hash legacy plaintext on first successful login.
IntelliJob/User/Profile.aspx.cs: hash newly provided profile passwords with `Utils.CreateSaltedHash` before saving; preserve existing stored password when unchanged.
IntelliJob/User/ResumeEnhancer.aspx: replaced static fixed-count education/experience/project cards with PlaceHolder controls (phEduCards, phExpCards, phProjCards) and Add buttons (btnAddEdu, btnAddExp, btnAddProj) in the section headings.
IntelliJob/User/ResumeEnhancer.aspx.designer.cs: removed all static per-index edu/exp/proj TextBox, DropDownList, and CheckBox field declarations; added PlaceHolder fields (phEduCards, phExpCards, phProjCards), Add button fields (btnAddEdu, btnAddExp, btnAddProj), and HiddenField count fields (hfEduCount, hfExpCount, hfProjCount).
IntelliJob/User/ResumeEnhancer.Structured.partial.cs: rewrote structured editor to use fully dynamic card creation via Page_Init/BuildEduCards/BuildExpCards/BuildProjCards so controls participate in ViewState; add-click handlers increment counts and rebuild; delete button on each card header collects remaining values, drops the targeted card, and repopulates; card counts are unlimited (no upper bound); SetEnhancerEditorsReadOnly now also toggles Add and Delete buttons.
IntelliJob/User/ResumeEnhancer.Structured.partial.cs: fixed card data loss on Add/Delete by moving all count adjustment logic into Page_Init using Request.Form detection; Page_Init now detects btnAddEdu/btnAddExp/btnAddProj/btnDel_* from form keys before controls exist, adjusts the final count, and builds exactly the right number of controls once — ViewState then restores existing card values automatically; Delete reads all current card values from Request.Form, drops the target slot, and stores shifted data in _pendingEduData/_pendingExpData/_pendingProjData; click handlers reduced to setting PreviewEditMode only.
IntelliJob/User/ResumeEnhancer.aspx.cs: added ApplyPendingCardData() call in Page_Load on postback path, invoked after ViewState is restored, so delete-shifted card values are written into the correct controls.
IntelliJob/User/ResumeEnhancer.aspx: added EnableViewState="false" on hfEduCount, hfExpCount, hfProjCount so ViewState never silently restores the old card count after Page_Init sets the new one.
IntelliJob/User/ResumeEnhancer.Structured.partial.cs: fixed second-add and cross-section-add bugs caused by IPostBackDataHandler overwriting HiddenField.Value with the submitted old count after Page_Init; Page_Init now stores final counts in _finalEduCount/_finalExpCount/_finalProjCount instance fields; ApplyPendingCardData (called in Page_Load) re-stamps the HiddenFields with these values after IPostBackDataHandler has run, ensuring the rendered HTML always carries the correct count for the next postback.
IntelliJob/User/ResumeEnhancer.Structured.partial.cs: removed Math.Max(1, ...) floor from delete count adjustments so users can delete all cards in a section; also fixed GetHiddenCount and ReadFormCount to accept 0 so save/parse loops correctly produce empty lists when all cards are deleted.
IntelliJob/User/ResumeEnhancer.aspx.cs: fixed MergeDocumentDefaults to treat an empty list as an intentional user deletion rather than a missing value; previously Count == 0 triggered fallback to the old saved data, silently restoring deleted sections; now only null triggers fallback so deleting all education/experience/project cards is correctly preserved through save and preview.
IntelliJob/User/ResumeEnhancer.Structured.partial.cs: removed Math.Max(1, ...) floors from PopulateStructuredPreview so that when a document is loaded with an empty education/experience/projects list (because the user deleted all cards and saved), zero cards are rendered and the user must explicitly click Add to create new ones.
IntelliJob/User/ResumeEnhancer.Structured.partial.cs: removed Grade and Coursework fields from education cards (UI, form collection, control population, parse/save path, flatten helper, and EduCardData struct) as they are not used in resume preview; changed project description field label to "Bullets (one per line)" matching experience, now parsed via SplitBullets into Bullets list on save, and ProjCardData.From prefers Bullets over flat Description when loading.
IntelliJob/User/ResumeEnhancer.Structured.partial.cs: moved Add buttons (Education/Experience/Project) from static section headings into the card header of the last card in each section; BuildCardHeader now accepts isLast, addBtnId, addLabel, addHandler parameters and renders a card-add-btn alongside the delete button inside a card-header-actions group; card factory methods (CreateEduCard, CreateExpCard, CreateProjCard) updated with isLast bool; BuildEduCards/BuildExpCards/BuildProjCards pass isLast=true for the final iteration; SetEnhancerEditorsReadOnly updated to toggle dynamic Add buttons by ID via EnumerateControlsRecursive.
IntelliJob/User/ResumeEnhancer.aspx: removed Add Education/Experience/Project buttons from section heading rows (buttons now live inside cards); replaced card-delete-btn and added card-add-btn and card-header-actions CSS with polished pill-style button design — Add button uses pink gradient with drop shadow, Delete button uses neutral outlined style with red hover state.
IntelliJob/User/ResumeEnhancer.aspx.designer.cs: removed static btnAddEdu, btnAddExp, btnAddProj field declarations as these buttons are now created dynamically in code-behind.
